...
export default function SeventhSection() { 
  ...
  const { mainItem, remainingItems } = useMemo(() => {
    ...
    const main = prodData.find(t => t.id === activeId) || prodData[0];
    const others = prodData.filter(t => t.id !== main.id);
      ...
      <div className="max-w-[1440px] mx-auto w-full px-24 flex h-full gap-12 pt-14">
        ...
        {/* Left Column */}
          <div className="absolute top-8 left-12 z-50 flex items-center gap-4">
            <button onClick={() => navigate('/#prod')} className="flex items-center justify-center w-10 h-10 bg-transparent text-[#f4efe6] border border-[#f4efe6] rounded-full hover:bg-[#f4efe6] hover:text-[#791220] transition-all group shadow-sm hover:shadow-md">
            ...
            <div className="text-[#f4efe6] font-sans font-bold text-[11px] uppercase tracking-[0.2em] flex items-center gap-2">
              ...
              <span className="opacity-50 hover:opacity-100 cursor-pointer transition-opacity" onClick={() => navigate('/#prod')}>TESTIMONIAL</span>
              <span className="opacity-50">/</span>
              <span className="opacity-100">{mainItem.name}</span>
            </div>
          </div>
        <div className="w-[35%] flex flex-col h-full shrink-0">
          <h2 className="text-[2rem] font-bold uppercase tracking-tight border-b-[3px] border-[#f4efe6] inline-block pb-1 mb-2 w-fit leading-none whitespace-nowrap">
            PRODUCED BY 14CREW
          </h2>
          ...
          {/* Grid 3x3 */}
          <div className="grid grid-cols-3 gap-3 w-full mb-4 relative mt-16">
        ...
        <div className="w-[65%] flex flex-col h-full border-l-[1px] border-[#f4efe6]/30 pl-12 py-2">
          ...
          {/* Main Clip */}
          <div data-cursor-text="PLAY" className="w-full aspect-[21/9] bg-gray-900 relative mb-4 shadow-2xl overflow-hidden border-[1px] border-white/10 group cursor-pointer z-10">
